...
class CctbiPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('ctbi.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ctbi`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ctbi WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ctbi (date, title, link, content)"
